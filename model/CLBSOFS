...
% Closed-Loop Brain Storm Optimization£¨CLBSO£©for feature selection
clc
    ...
    for idx = 1:n_p
        ...
        if r_1 < prob_one_cluster % select one cluster
                ...
                if r < prob(idj,1)                      
                    if rand() < 0.4  % use the center
                       indi_temp(1,:) = centers(idj,:); 
                        ...
                        [FFFF,x22]=min(fitness_popu);
                        indi_1 = acculate_num_cluster(idj,1) + ceil(rand() * number_in_cluster(idj,1));
                        indi_2 = acculate_num_cluster(idj,1) + ceil(rand() * number_in_cluster(idj,1));
                        indi_temp(1,:) = popu(idx,:)+(popu(x22,:)-popu_sorted(indi_1,:))*normrnd(0,1,1,n_d); %%case'DB' 
                        indi_temp(1,:) = popu(idx,:)+(popu_sorted(indi_2,:)-popu_sorted(indi_1,:))*normrnd(0,1,1,n_d); %%case'RS' 
                        x33=randperm(n_c);
                        indi_temp(1,:) = popu(idx,:)+(centers(x33(1),:)-centers(x33(2),:))*normrnd(0,1,1,n_d); %%case'RGC' 
                    end
                    break
                end
            end
            ...
            if rand() < 0.5 %use center
                indi_temp(1,:) = tem * centers(cluster_1,:) + (1-tem) * centers(cluster_2,:)+ stepSize .* normrnd(0,1,1,n_d);
            else   % use randomly selected individuals from each cluster            
                indi_temp(1,:) = tem * popu_sorted(indi_1,:) + (1-tem) * popu_sorted(indi_2,:)+ stepSize .* normrnd(0,1,1,n_d);%
            end
        end                 
