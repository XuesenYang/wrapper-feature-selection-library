...
while n_iteration < max_iteration
    rndNum=diff([0,sort(randperm(n_p+n_c-1,n_c-1)),n_p+n_c])-1;%%%%
    while size(find(rndNum<=3))>=1
    rndNum=diff([0,sort(randperm(n_p+n_c-1,n_c-1)),n_p+n_c])-1;%%%%
    end
    [sA,index] = sort(fitness_popu);
   acculate_num_cluster = zeros(n_c,1);
   for idx =2:n_c
         acculate_num_cluster(idx,1) = acculate_num_cluster((idx-1),1) +  rndNum(1,(idx-1));%%%
   end
   for id =1:n_c
       anumber=index((acculate_num_cluster(id,1)+1):(acculate_num_cluster(id,1)+rndNum(1,id)));
       bnumber=ones(rndNum(id),1);
       cluster(anumber,1)=id*bnumber;
   end
    % clustering    
...
%             best(cluster(idx,1),1) = idx;
        ...
        for ic=1:n_c
            best(ic,1)=index((acculate_num_cluster(ic,1)+1),1);
        end
    end  
    best;
    ...
    save('MBSOFSV2','Best_fit','BESTFIT')
