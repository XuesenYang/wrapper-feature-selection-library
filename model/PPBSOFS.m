...
% Predator prey Brain Storm Optimization (PPBSO) for feature selection
clc
    ...
    for idx = 1:n_p
        ...
        if r_1 < prob_one_cluster % select one cluster
            ...
            if rand() < 0.5 %use center
                indi_temp(1,:) =popu(idx,:)+0.05*(popu(x22,:)-popu(idx,:));
            else   % use randomly selected individuals from each cluster            
                indi_temp(1,:) = popu(idx,:)+0.1*(rang_r-rang_l)*sign( centers(cluster_1,:)- popu(idx,:))*exp(100/(rang_r-rang_l)*abs(centers(cluster_1,:)- popu(idx,:)));
            end
        end                 
